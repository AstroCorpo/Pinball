...
def load_map(name) :
    global WALLS, BALLS, BALL, BALL_SHAPE, BALL_PARAMS, RIGHT_FLIPPERS, LEFT_FLIPPERS, BLOCKADE, BLOCKADE_SHAPE, BLOCKADE_PARAMS, WIDTH, HEIGHT
...
def at_start() :
    global BALL, TUNNEL_SIZE
    
    if distance_points((BALL_POSITION_X, BALL_POSITION_Y), BALL.position) > TUNNEL_SIZE :
        return False
    return True


def run(preset="fancy"):
    global BALL, BALL_SHAPE, BLOCKADE, BLOCKADE_SHAPE, POINTS, BASE_WIDTH, TUNNEL_SIZE, WIDTH, breaking_point, WALLS, OBSTACLES, LAUNCH_ENERGY, BALL_POSITION_X, BALL_POSITION_Y
    ...
    while RUNNING:
        ...
        for event in pygame.event.get():
            if event.type == pygame.QUIT:
                ...
                if event.key == pygame.K_RIGHT:
                    ...
                    if at_start() :
                        BALL.apply_impulse_at_local_point((0, -100*energy_stored))
                    space_pressed = False
        ...
        draw_text_inside(SCREEN, f"FPS: {text_fps}", (0, 0), BLACK)
